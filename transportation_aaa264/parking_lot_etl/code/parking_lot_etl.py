...
taxi_zones = pd.read_csv('/Volumes/恢复/732实验/project/taxi_zones.csv')
g_taxi_zones = gpd.GeoDataFrame(taxi_zones)
g_taxi_zones['geometry'] = gpd.GeoSeries.from_wkt(g_taxi_zones['the_geom'])


lots = pd.read_csv('/Volumes/恢复/732实验/project/DOITT_PARKING_LOT.csv')
...
g_lots.to_parquet('lots_with_location.parquet', index=False)
